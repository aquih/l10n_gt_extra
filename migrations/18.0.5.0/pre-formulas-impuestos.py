...
def migrate(cr, version):
    ...
    cr.execute("update account_tax set formula = 'base/1.12 * -0.05', amount_type = 'percent', amount = -5, price_include_override = 'tax_excluded' where id in (select res_id from ir_model_data where name like '%impuestos%_isr_factura_especial')")
